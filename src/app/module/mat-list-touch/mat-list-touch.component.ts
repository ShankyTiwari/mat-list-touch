...
export class MatListTouchComponent implements OnInit {
  ...
  @Input() separatorEval?: (item: any, index: number) => boolean;
  @Input() leftBorderEval?: (item: any, index: number) => string;
  @Input() rightBorderEval?: (item: any, index: number) => string;
  @Input() disableActionsEval: (item: any, index: number) => boolean;
  ...
  evalLeftBorder(item: any, index: number) {
    if (this.leftBorderEval) {
      return this.leftBorderEval(item, index);
    }
    return '';
  }

  evalRightBorder(item: any, index: number) {
    if (this.rightBorderEval) {
      return this.rightBorderEval(item, index);
    }
    return '';
  }
  ...
  panMoveEvent(action, elementRef, item: object, index: number): void {
    if (this.disableActionsEval && this.disableActionsEval(item, index)) {
  ...
  panEndEvent(event, item: object, index: number): void {
